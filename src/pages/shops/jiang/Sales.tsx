...
// Mock data สำหรับร้านเจียง - แยกตามเมนู
const initialSalesData = [
  {
    id: "1",
    date: "2024-12-15",
    items: [
      { name: "ก๋วยเตี๋ยวลูกชิ้นปลา", quantity: 20, price: 55, total: 1100, menu: "ก๋วยเตี๋ยว" },
      { name: "เย็นตาโฟ", quantity: 10, price: 60, total: 600, menu: "เย็นตาโฟ" },
    ],
    total: 1700,
  ...
  },
  {
    ...
    date: "2024-12-15",
    items: [
      { name: "ก๋วยเตี๋ยวลูกชิ้นปลา", quantity: 15, price: 55, total: 825, menu: "ก๋วยเตี๋ยว" },
      { name: "ขนมจีนบุฟเฟ่ต์", quantity: 5, price: 120, total: 600, menu: "อื่นๆ" },
    ],
    total: 1425,
    paymentMethod: "บัตร",
    customer: "ลูกค้าทั่วไป",
  ...
  },
  {
    ...
    date: "2024-12-14",
    items: [
      { name: "ก๋วยเตี๋ยวลูกชิ้นปลา", quantity: 50, price: 55, total: 2750, menu: "ก๋วยเตี๋ยว" },
      { name: "เย็นตาโฟ", quantity: 30, price: 60, total: 1800, menu: "เย็นตาโฟ" },
      { name: "ขนมจีนบุฟเฟ่ต์", quantity: 10, price: 120, total: 1200, menu: "อื่นๆ" },
    ],
    total: 5750,
...
export default function Sales() {
  ...
  const [formData, setFormData] = useState({
    ...
    items: [{ name: "", quantity: "", price: "", total: 0, menu: "ก๋วยเตี๋ยว" }],
    paymentMethod: "เงินสด",
    customer: "",
  });
  ...
  const addItemToForm = () => {
    setFormData({
      ...formData,
      items: [...formData.items, { name: "", quantity: "", price: "", total: 0, menu: "ก๋วยเตี๋ยว" }],
    });
  };
  ...
  const menus = ["ก๋วยเตี๋ยว", "เย็นตาโฟ", "อื่นๆ"];
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="text-muted font-light">
          บันทึกยอดขายและดูรายงานยอดขายรายวัน/เดือน/ปี แยกตามเมนู (ก๋วยเตี๋ยว, เย็นตาโฟ) เปรียบเทียบช่วงเวลา
        </p>
      </motion.div>
      ...
      <motion.div
        ...
        className="panel rounded-2xl p-6"
      >
        <h3 className="text-xl font-semibold text-app mb-4">ยอดขายแยกตามเมนู</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-orange-500/10 border border-orange-500/30 rounded-xl p-4">
            <p className="text-sm text-muted mb-1">ก๋วยเตี๋ยวลูกชิ้นปลา</p>
            <p className="text-2xl font-bold text-orange-400">
              {currencyFormatter.format(salesByMenu["ก๋วยเตี๋ยว"] || 0)}
            </p>
            <p className="text-xs text-muted mt-1">
              {totalSales > 0 ? ((salesByMenu["ก๋วยเตี๋ยว"] || 0) / totalSales * 100).toFixed(1) : 0}% ของยอดขาย
            </p>
          </div>
          <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4">
            <p className="text-sm text-muted mb-1">เย็นตาโฟ</p>
            <p className="text-2xl font-bold text-blue-400">
              {currencyFormatter.format(salesByMenu["เย็นตาโฟ"] || 0)}
            </p>
            <p className="text-xs text-muted mt-1">
              {totalSales > 0 ? ((salesByMenu["เย็นตาโฟ"] || 0) / totalSales * 100).toFixed(1) : 0}% ของยอดขาย
            </p>
          </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
        ...
        <div className="flex gap-2">
          <label htmlFor="jiang-sales-upload-stock" className="flex items-center gap-2 px-4 py-2 bg-soft text-app rounded-lg hover:bg-app/10 transition-colors cursor-pointer">
            <Upload className="w-4 h-4" />
            <span>นำเข้าจาก Stock Program</span>
            <input
              id="jiang-sales-upload-stock"
              type="file"
              accept=".xlsx,.xls"
      ...
      >
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="jiang-sales-date" className="block text-sm font-medium text-app mb-2">วันที่</label>
              <input
                id="jiang-sales-date"
                type="date"
                value={formData.date}
            ...
            <div>
              <label htmlFor="jiang-sales-payment" className="block text-sm font-medium text-app mb-2">วิธีชำระ</label>
              <select
                id="jiang-sales-payment"
          ...
          <div>
            <label htmlFor="jiang-sales-customer" className="block text-sm font-medium text-app mb-2">ลูกค้า (ไม่บังคับ)</label>
            <input
              id="jiang-sales-customer"
              type="text"
