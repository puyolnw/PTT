...
const mockSalesData = {
  ...
  lastMonth: 470000,
  byMenu: {
    kuayTiew: { amount: 10800, percentage: 60, name: "ก๋วยเตี๋ยวลูกชิ้นปลา" },
    yenTaFo: { amount: 5400, percentage: 30, name: "เย็นตาโฟ" },
    others: { amount: 1800, percentage: 10, name: "อื่นๆ" },
  },
};

const mockStockData = [
  { name: "ลูกชิ้นปลา", quantity: 50, unit: "กก.", lowStock: true, expiry: "2024-12-20", category: "วัตถุดิบทะเล" },
  { name: "เส้นก๋วยเตี๋ยว", quantity: 30, unit: "กก.", lowStock: true, expiry: "2025-01-15", category: "วัตถุดิบ" },
  { name: "ปลาสด", quantity: 25, unit: "กก.", lowStock: true, expiry: "2024-12-18", category: "วัตถุดิบทะเล" },
  { name: "น้ำซุป", quantity: 100, unit: "ลิตร", lowStock: false, expiry: "2025-01-10", category: "วัตถุดิบ" },
  { name: "ผักกาด", quantity: 15, unit: "กก.", lowStock: true, expiry: "2024-12-19", category: "วัตถุดิบ" },
];
...
const mockRecentPurchases = [
  { id: "1", supplier: "ตลาดทะเล", item: "ปลาสด 50 กก.", amount: 25000, date: "2024-12-15", status: "ชำระแล้ว", source: "Stock Program" },
  { id: "2", supplier: "PTT", item: "ค่าน้ำ/ไฟ", amount: 3000, date: "2024-12-10", status: "ชำระแล้ว", source: "Manual" },
  { id: "3", supplier: "ซัพพลายเออร์ A", item: "ลูกชิ้นปลา 100 กก.", amount: 35000, date: "2024-12-08", status: "รอชำระ", source: "Stock Program" },
];

const mockTopSellingItems = [
  { name: "เย็นตาโฟ", sales: 162000, quantity: 3000, trend: "+15%", menu: "เย็นตาโฟ" },
  { name: "ก๋วยเตี๋ยวลูกชิ้นปลา", sales: 324000, quantity: 6000, trend: "+8%", menu: "ก๋วยเตี๋ยว" },
  { name: "ขนมจีนบุฟเฟ่ต์", sales: 54000, quantity: 500, trend: "+25%", menu: "อื่นๆ" },
];

export default function JiangDashboard() {
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="text-muted font-light">
          ภาพรวมยอดขาย สต็อกสินค้า และการเงินของร้านอาหารก๋วยเตี๋ยวลูกชิ้นปลา (ผลิตตามมาตรฐาน GMP/HACCP)
        </p>
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-muted">ยอดขายวันนี้</p>
          </div>
          <BarChart3 className="w-6 h-6 text-muted" />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-orange-500/10 border border-orange-500/30 rounded-xl p-4">
            <p className="text-sm text-muted mb-1">{mockSalesData.byMenu.kuayTiew.name}</p>
            <p className="text-2xl font-bold text-orange-400">
              {currencyFormatter.format(mockSalesData.byMenu.kuayTiew.amount)}
            </p>
            <p className="text-xs text-muted mt-1">
              {mockSalesData.byMenu.kuayTiew.percentage}% ของยอดขาย
            </p>
          </div>
          <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4">
            <p className="text-sm text-muted mb-1">{mockSalesData.byMenu.yenTaFo.name}</p>
            <p className="text-2xl font-bold text-blue-400">
              {currencyFormatter.format(mockSalesData.byMenu.yenTaFo.amount)}
            </p>
            <p className="text-xs text-muted mt-1">
              {mockSalesData.byMenu.yenTaFo.percentage}% ของยอดขาย
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-xl font-semibold text-app">สต็อกวัตถุดิบ</h3>
              <p className="text-sm text-muted">สต็อกคงเหลือและสถานะ (เน้นวัตถุดิบทะเล)</p>
            </div>
            <Fish className="w-6 h-6 text-muted" />
          </div>
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-muted">ยอดขายรายเดือน</p>
            </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted">
                      {numberFormatter.format(item.quantity)} จาน • {item.menu}
                    </p>
                  </div>
                </div>
                <div className="text-right">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-muted">รายการล่าสุด (วัตถุดิบทะเล)</p>
            </div>
