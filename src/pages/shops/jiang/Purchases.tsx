  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      const newPurchase = {
        ...
        date: new Date().toISOString().split("T")[0],
        supplier: "ตลาดทะเล",
        items: `วัตถุดิบทะเลจากไฟล์ Stock Program`,
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="text-muted font-light">
          บันทึกการซื้อวัตถุดิบทะเล (ปลาสด, ลูกชิ้นปลา) จากซัพพลายเออร์ นำเข้าจาก Stock Program หรือกรอกด้วยมือ
        </p>
      </motion.div>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
        ...
        <div className="flex gap-2">
          <label htmlFor="jiang-upload-stock" className="flex items-center gap-2 px-4 py-2 bg-soft text-app rounded-lg hover:bg-app/10 transition-colors cursor-pointer">
            <Upload className="w-4 h-4" />
            <span>นำเข้าจาก Stock Program</span>
            <input
              id="jiang-upload-stock"
              type="file"
              accept=".xlsx,.xls"
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <Fish className="w-5 h-5 text-ptt-cyan" />
      ...
      >
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="jiang-purchase-date" className="block text-sm font-medium text-app mb-2">วันที่</label>
              <input
                id="jiang-purchase-date"
                type="date"
                value={formData.date}
            ...
            <div>
              <label htmlFor="jiang-purchase-status" className="block text-sm font-medium text-app mb-2">สถานะ</label>
              <select
                id="jiang-purchase-status"
          ...
          <div>
            <label htmlFor="jiang-purchase-supplier" className="block text-sm font-medium text-app mb-2">ซัพพลายเออร์</label>
            <input
              id="jiang-purchase-supplier"
              type="text"
              value={formData.supplier}
              onChange={(e) => setFormData({ ...formData, supplier: e.target.value })}
              className="w-full px-4 py-2 bg-soft border border-app rounded-lg text-app"
              placeholder="เช่น ตลาดทะเล, PTT, ตลาดสด"
              required
            />
          </div>
          <div>
            <label htmlFor="jiang-purchase-items" className="block text-sm font-medium text-app mb-2">รายการสินค้า</label>
            <textarea
              id="jiang-purchase-items"
              value={formData.items}
              onChange={(e) => setFormData({ ...formData, items: e.target.value })}
              className="w-full px-4 py-2 bg-soft border border-app rounded-lg text-app"
              placeholder="เช่น ปลาสด 50 กก., ลูกชิ้นปลา 100 กก., ค่าน้ำ/ไฟ"
              rows={3}
              required
            />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="jiang-purchase-amount" className="block text-sm font-medium text-app mb-2">จำนวนเงิน (บาท)</label>
              <input
                id="jiang-purchase-amount"
                type="number"
            ...
            {formData.status === "รอชำระ" && (
              <div>
                <label htmlFor="jiang-purchase-due-date" className="block text-sm font-medium text-app mb-2">ครบกำหนด</label>
                <input
                  id="jiang-purchase-due-date"
                  type="date"
          ...
          <div>
            <label htmlFor="jiang-purchase-source" className="block text-sm font-medium text-app mb-2">แหล่งที่มา</label>
            <select
              id="jiang-purchase-source"
