...
const initialSalesData = [
  {
    ...
    date: "2024-12-15",
    items: [
      { name: "กาแฟเย็น", quantity: 50, price: 25, total: 1250, category: "เครื่องดื่ม" },
      { name: "ขนมปัง", quantity: 30, price: 15, total: 450, category: "อาหาร" },
      { name: "น้ำอัดลม", quantity: 20, price: 20, total: 400, category: "เครื่องดื่ม" },
    ],
    total: 2100,
  ...
  },
  {
    ...
    date: "2024-12-15",
    items: [
      { name: "บะหมี่กึ่งสำเร็จรูป", quantity: 10, price: 15, total: 150, category: "อาหาร" },
      { name: "นม UHT", quantity: 15, price: 12, total: 180, category: "เครื่องดื่ม" },
    ],
    total: 330,
    paymentMethod: "บัตร",
    customer: "ลูกค้าทั่วไป",
  ...
  },
  {
    ...
    date: "2024-12-14",
    items: [
      { name: "กาแฟเย็น", quantity: 100, price: 25, total: 2500, category: "เครื่องดื่ม" },
      { name: "ขนมปัง", quantity: 80, price: 15, total: 1200, category: "อาหาร" },
      { name: "น้ำอัดลม", quantity: 50, price: 20, total: 1000, category: "เครื่องดื่ม" },
      { name: "บะหมี่กึ่งสำเร็จรูป", quantity: 30, price: 15, total: 450, category: "อาหาร" },
    ],
    total: 5150,
...
export default function Sales() {
  ...
  const [formData, setFormData] = useState({
    ...
    items: [{ name: "", quantity: "", price: "", total: 0, category: "อาหาร" }],
    paymentMethod: "เงินสด",
    customer: "",
  });
  ...
  const addItemToForm = () => {
    setFormData({
      ...formData,
      items: [...formData.items, { name: "", quantity: "", price: "", total: 0, category: "อาหาร" }],
    });
  };
  ...
  const categories = ["อาหาร", "เครื่องดื่ม", "อื่นๆ"];

  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (file) {
      // Simulate file processing
      alert(`กำลังประมวลผลไฟล์ ${file.name}...\n\nระบบจะนำเข้าข้อมูลยอดขายจากแอป 7-Eleven`);
    }
  };
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="text-muted font-light">
          บันทึกยอดขายและดูรายงานยอดขายรายวัน/เดือน/ปี แยกตามหมวด (อาหาร, เครื่องดื่ม, อื่นๆ)
        </p>
      </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm text-muted">จาก Excel/PDF</span>
          </div>
          <p className="text-2xl font-bold text-app">
            {salesData.filter((s) => s.source === "Excel" || s.source === "PDF").length}
          </p>
      ...
      <motion.div
        ...
        className="panel rounded-2xl p-6"
      >
        <h3 className="text-xl font-semibold text-app mb-4">ยอดขายแยกตามหมวด</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-orange-500/10 border border-orange-500/30 rounded-xl p-4">
            <p className="text-sm text-muted mb-1">อาหาร</p>
            <p className="text-2xl font-bold text-orange-400">
              {currencyFormatter.format(salesByCategory["อาหาร"] || 0)}
            </p>
            <p className="text-xs text-muted mt-1">
              {totalSales > 0 ? ((salesByCategory["อาหาร"] || 0) / totalSales * 100).toFixed(1) : 0}% ของยอดขาย
            </p>
          </div>
          <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4">
            <p className="text-sm text-muted mb-1">เครื่องดื่ม</p>
            <p className="text-2xl font-bold text-blue-400">
              {currencyFormatter.format(salesByCategory["เครื่องดื่ม"] || 0)}
            </p>
            <p className="text-xs text-muted mt-1">
              {totalSales > 0 ? ((salesByCategory["เครื่องดื่ม"] || 0) / totalSales * 100).toFixed(1) : 0}% ของยอดขาย
            </p>
          </div>
          <div className="bg-purple-500/10 border border-purple-500/30 rounded-xl p-4">
            <p className="text-sm text-muted mb-1">อื่นๆ</p>
            <p className="text-2xl font-bold text-purple-400">
              {currencyFormatter.format(salesByCategory["อื่นๆ"] || 0)}
            </p>
            <p className="text-xs text-muted mt-1">
              {totalSales > 0 ? ((salesByCategory["อื่นๆ"] || 0) / totalSales * 100).toFixed(1) : 0}% ของยอดขาย
            </p>
          </div>
        </div>
      </motion.div>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
        ...
        <div className="flex gap-2">
          <label htmlFor="seven-sales-upload" className="flex items-center gap-2 px-4 py-2 bg-soft text-app rounded-lg hover:bg-app/10 transition-colors cursor-pointer">
            <Upload className="w-4 h-4" />
            <span>นำเข้า Excel/PDF</span>
            <input
              id="seven-sales-upload"
              type="file"
              accept=".xlsx,.xls,.pdf"
      ...
      >
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="seven-sales-date" className="block text-sm font-medium text-app mb-2">วันที่</label>
              <input
                id="seven-sales-date"
                type="date"
                value={formData.date}
            ...
            <div>
              <label htmlFor="seven-sales-payment" className="block text-sm font-medium text-app mb-2">วิธีชำระ</label>
              <select
                id="seven-sales-payment"
          ...
          <div>
            <label htmlFor="seven-sales-customer" className="block text-sm font-medium text-app mb-2">ลูกค้า (ไม่บังคับ)</label>
            <input
              id="seven-sales-customer"
              type="text"
