...
const initialPurchases = [
  {
    ...
    date: "2024-12-15",
    supplier: "CP All",
    items: "สินค้าจากแคตตาล็อก (กาแฟเย็น, ขนมปัง, น้ำอัดลม)",
    amount: 50000,
    status: "ชำระแล้ว",
    dueDate: "2024-12-15",
    source: "Excel",
  ...
  },
  {
    ...
    date: "2024-12-08",
    supplier: "CP All",
    items: "สินค้าโปรโมชัน (บะหมี่กึ่งสำเร็จรูป, นม UHT)",
    amount: 25000,
    status: "รอชำระ",
    dueDate: "2024-12-20",
    source: "PDF",
...
export default function Purchases() {
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>, fileType: "excel" | "pdf") => {
    ...
    if (file) {
      ...
      const newPurchase = {
        ...
        date: new Date().toISOString().split("T")[0],
        supplier: "CP All",
        items: `สินค้าจากไฟล์ ${fileType.toUpperCase()}`,
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm text-muted">จากไฟล์</span>
          </div>
          <p className="text-2xl font-bold text-app">
            {purchases.filter((p) => p.source === "Excel" || p.source === "PDF").length}
          </p>
          <p className="text-sm text-muted">รายการ</p>
        </motion.div>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-4 py-2 bg-soft border border-app rounded-lg text-app"
              placeholder="เช่น สินค้าจากแคตตาล็อก, ค่าน้ำ/ไฟ"
              rows={3}
              required
            />
          </div>
