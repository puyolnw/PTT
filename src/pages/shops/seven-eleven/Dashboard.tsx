...
const mockSalesData = {
  ...
  lastMonth: 1260000,
  byCategory: {
    food: { amount: 27000, percentage: 60 },
    drinks: { amount: 13500, percentage: 30 },
    others: { amount: 4500, percentage: 10 },
  },
};
...
const mockRecentPurchases = [
  { id: "1", supplier: "CP All", item: "สินค้าจากแคตตาล็อก", amount: 50000, date: "2024-12-15", status: "ชำระแล้ว", source: "Excel" },
  { id: "2", supplier: "PTT", item: "ค่าน้ำ/ไฟ", amount: 3000, date: "2024-12-10", status: "ชำระแล้ว", source: "Manual" },
  { id: "3", supplier: "CP All", item: "สินค้าโปรโมชัน", amount: 25000, date: "2024-12-08", status: "รอชำระ", source: "PDF" },
];

const mockTopSellingItems = [
  { name: "กาแฟเย็น", sales: 15000, quantity: 200, trend: "+15%", category: "เครื่องดื่ม" },
  { name: "ขนมปัง", sales: 12000, quantity: 150, trend: "+8%", category: "อาหาร" },
  { name: "น้ำอัดลม", sales: 8000, quantity: 100, trend: "-5%", category: "เครื่องดื่ม" },
  { name: "บะหมี่กึ่งสำเร็จรูป", sales: 5000, quantity: 80, trend: "-12%", category: "อาหาร" },
];
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-orange-500/10 border border-orange-500/30 rounded-xl p-4">
            ...
            <p className="text-2xl font-bold text-orange-400">
              {currencyFormatter.format(mockSalesData.byCategory.food.amount)}
            </p>
            <p className="text-xs text-muted mt-1">
              {mockSalesData.byCategory.food.percentage}% ของยอดขาย
            </p>
          </div>
          <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4">
            <p className="text-sm text-muted mb-1">เครื่องดื่ม</p>
            <p className="text-2xl font-bold text-blue-400">
              {currencyFormatter.format(mockSalesData.byCategory.drinks.amount)}
            </p>
            <p className="text-xs text-muted mt-1">
              {mockSalesData.byCategory.drinks.percentage}% ของยอดขาย
            </p>
          </div>
          <div className="bg-purple-500/10 border border-purple-500/30 rounded-xl p-4">
            <p className="text-sm text-muted mb-1">อื่นๆ</p>
            <p className="text-2xl font-bold text-purple-400">
              {currencyFormatter.format(mockSalesData.byCategory.others.amount)}
            </p>
            <p className="text-xs text-muted mt-1">
              {mockSalesData.byCategory.others.percentage}% ของยอดขาย
            </p>
          </div>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <button className="flex items-center gap-3 p-4 bg-ptt-blue/10 hover:bg-ptt-blue/20 border border-ptt-blue/30 rounded-xl transition-colors">
            ...
            <span className="text-app font-medium">นำเข้า Excel/PDF</span>
          </button>
