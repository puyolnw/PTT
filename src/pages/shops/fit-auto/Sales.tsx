...
// Mock data สำหรับ FIT Auto - แยกตามบริการ
const initialSalesData = [
  {
    id: "1",
    date: "2024-12-15",
    items: [
      { name: "เปลี่ยนน้ำมันเครื่อง PTT Lubricants", quantity: 1, price: 1500, total: 1500, service: "เปลี่ยนน้ำมันเครื่อง" },
      { name: "ไส้กรองน้ำมันเครื่อง", quantity: 1, price: 300, total: 300, service: "เปลี่ยนน้ำมันเครื่อง" },
    ],
    total: 1800,
  ...
  },
  {
    ...
    date: "2024-12-15",
    items: [
      { name: "ซ่อมยาง", quantity: 1, price: 800, total: 800, service: "ซ่อมยาง" },
      { name: "ค่าแรงช่าง", quantity: 1, price: 200, total: 200, service: "ซ่อมยาง" },
    ],
    total: 1000,
  ...
  },
  {
    ...
    items: [
      ...
      { name: "บริการ EV", quantity: 1, price: 2000, total: 2000, service: "อื่นๆ" },
    ],
    total: 2500,
...
export default function Sales() {
  ...
  const [formData, setFormData] = useState({
    ...
    items: [{ name: "", quantity: "", price: "", total: 0, service: "เปลี่ยนน้ำมันเครื่อง" }],
    paymentMethod: "เงินสด",
    customer: "",
  });
  ...
  const addItemToForm = () => {
    setFormData({
      ...formData,
      items: [...formData.items, { name: "", quantity: "", price: "", total: 0, service: "เปลี่ยนน้ำมันเครื่อง" }],
    });
  };
  ...
  const services = ["เปลี่ยนน้ำมันเครื่อง", "ซ่อมยาง", "ตรวจสภาพรถ", "อื่นๆ"];
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="text-muted font-light">
          บันทึกยอดขายและดูรายงานยอดขายรายวัน/เดือน/ปี แยกตามบริการ (เปลี่ยนน้ำมัน, ซ่อมยาง, ตรวจสภาพรถ) เปรียบเทียบช่วงเวลา เชื่อมกับ M1 สำหรับชำระเงิน
        </p>
      </motion.div>
      ...
      <motion.div
        ...
        className="panel rounded-2xl p-6"
      >
        <h3 className="text-xl font-semibold text-app mb-4">ยอดขายแยกตามบริการ (เปลี่ยนน้ำมัน 70%)</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-orange-500/10 border border-orange-500/30 rounded-xl p-4">
            <p className="text-sm text-muted mb-1">เปลี่ยนน้ำมันเครื่อง</p>
            <p className="text-2xl font-bold text-orange-400">
              {currencyFormatter.format(salesByService["เปลี่ยนน้ำมันเครื่อง"] || 0)}
            </p>
            <p className="text-xs text-muted mt-1">
              {totalSales > 0 ? ((salesByService["เปลี่ยนน้ำมันเครื่อง"] || 0) / totalSales * 100).toFixed(1) : 0}% ของยอดขาย
            </p>
          </div>
          <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4">
            <p className="text-sm text-muted mb-1">ซ่อมยาง</p>
            <p className="text-2xl font-bold text-blue-400">
              {currencyFormatter.format(salesByService["ซ่อมยาง"] || 0)}
            </p>
            <p className="text-xs text-muted mt-1">
              {totalSales > 0 ? ((salesByService["ซ่อมยาง"] || 0) / totalSales * 100).toFixed(1) : 0}% ของยอดขาย
            </p>
          </div>
          ...
          <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-4">
            ...
            <p className="text-2xl font-bold text-green-400">
              {currencyFormatter.format(salesByService["อื่นๆ"] || 0)}
            </p>
            <p className="text-xs text-muted mt-1">
              {totalSales > 0 ? ((salesByService["อื่นๆ"] || 0) / totalSales * 100).toFixed(1) : 0}% ของยอดขาย
            </p>
          </div>
        </div>
      </motion.div>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
        ...
        <div className="flex gap-2">
          <label htmlFor="fit-auto-sales-upload-stock" className="flex items-center gap-2 px-4 py-2 bg-soft text-app rounded-lg hover:bg-app/10 transition-colors cursor-pointer">
            <Upload className="w-4 h-4" />
            <span>นำเข้าจาก Stock Program</span>
            <input
              id="fit-auto-sales-upload-stock"
              type="file"
              accept=".xlsx,.xls"
      ...
      >
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="fit-auto-sales-date" className="block text-sm font-medium text-app mb-2">วันที่</label>
              <input
                id="fit-auto-sales-date"
                type="date"
                value={formData.date}
            ...
            <div>
              <label htmlFor="fit-auto-sales-payment" className="block text-sm font-medium text-app mb-2">วิธีชำระ</label>
              <select
                id="fit-auto-sales-payment"
          ...
          <div>
            <label htmlFor="fit-auto-sales-customer" className="block text-sm font-medium text-app mb-2">ลูกค้า (ไม่บังคับ)</label>
            <input
              id="fit-auto-sales-customer"
              type="text"
                ...
                <div key={index} className="grid grid-cols-12 gap-2 items-end">
                  ...
                  <div className="col-span-3">
                    ...
                    >
                      ...
                      <option value="ซ่อมยาง">ซ่อมยาง</option>
                      <option value="ตรวจสภาพรถ">ตรวจสภาพรถ</option>
                      <option value="อื่นๆ">อื่นๆ (EV)</option>
                    </select>
                  </div>
