...
// Mock data สำหรับร้านมอเตอร์ไซค์ไฟฟ้า - แยกตามสินค้า
const initialSalesData = [
  {
    id: "1",
    date: "2024-12-15",
    items: [
      { name: "สกู๊ตเตอร์ไฟฟ้า 800W", quantity: 1, price: 25000, total: 25000, product: "สกู๊ตเตอร์ไฟฟ้า" },
    ],
    total: 25000,
    paymentMethod: "ผ่อน 0% 12 เดือน",
  ...
  },
  {
    ...
    date: "2024-12-15",
    items: [
      { name: "มอเตอร์ไซค์ไฟฟ้า 800W", quantity: 1, price: 35000, total: 35000, product: "มอเตอร์ไซค์ไฟฟ้า" },
    ],
    total: 35000,
    paymentMethod: "บัตร",
    customer: "ลูกค้าทั่วไป",
  ...
  },
  {
    ...
    date: "2024-12-14",
    items: [
      { name: "จักรยานไฟฟ้า Sabaie Ice Cream", quantity: 1, price: 25000, total: 25000, product: "จักรยานไฟฟ้า" },
      { name: "แบตเตอรี่ Graphene 60V", quantity: 1, price: 8000, total: 8000, product: "อะไหล่ EV" },
    ],
    total: 33000,
...
export default function Sales() {
  ...
  const [formData, setFormData] = useState({
    ...
    items: [{ name: "", quantity: "", price: "", total: 0, product: "สกู๊ตเตอร์ไฟฟ้า" }],
  ...
  const filteredSales = salesData.filter((sale) => {
    ...
    const matchesProduct = !productFilter || sale.items.some((item) => item.product === productFilter);
    return matchesSearch && matchesDate && matchesPayment && matchesProduct;
  });
  ...
  const addItemToForm = () => {
    setFormData({
      ...formData,
      items: [...formData.items, { name: "", quantity: "", price: "", total: 0, product: "สกู๊ตเตอร์ไฟฟ้า" }],
    });
  };
  ...
  const products = ["สกู๊ตเตอร์ไฟฟ้า", "มอเตอร์ไซค์ไฟฟ้า", "จักรยานไฟฟ้า", "สามล้อไฟฟ้า", "อะไหล่ EV"];
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="text-muted font-light">
          บันทึกยอดขายและดูรายงานยอดขายรายวัน/เดือน/ปี แยกตามสินค้า (สกู๊ตเตอร์ไฟฟ้า, มอเตอร์ไซค์ไฟฟ้า, จักรยานไฟฟ้า, สามล้อไฟฟ้า) เปรียบเทียบช่วงเวลา รองรับโปรโมชันผ่อน 0% 12 เดือน เชื่อมกับ M1 สำหรับชำระเงิน
        </p>
      </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          <div className="bg-orange-500/10 border border-orange-500/30 rounded-xl p-4">
            ...
            <p className="text-2xl font-bold text-orange-400">
              {currencyFormatter.format(salesByProduct["สกู๊ตเตอร์ไฟฟ้า"] || 0)}
            </p>
            <p className="text-xs text-muted mt-1">
              {totalSales > 0 ? ((salesByProduct["สกู๊ตเตอร์ไฟฟ้า"] || 0) / totalSales * 100).toFixed(1) : 0}% ของยอดขาย
            </p>
          </div>
          <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4">
            <p className="text-sm text-muted mb-1">มอเตอร์ไซค์ไฟฟ้า</p>
            <p className="text-2xl font-bold text-blue-400">
              {currencyFormatter.format(salesByProduct["มอเตอร์ไซค์ไฟฟ้า"] || 0)}
            </p>
            <p className="text-xs text-muted mt-1">
              {totalSales > 0 ? ((salesByProduct["มอเตอร์ไซค์ไฟฟ้า"] || 0) / totalSales * 100).toFixed(1) : 0}% ของยอดขาย
            </p>
          </div>
          <div className="bg-purple-500/10 border border-purple-500/30 rounded-xl p-4">
            <p className="text-sm text-muted mb-1">จักรยานไฟฟ้า</p>
            <p className="text-2xl font-bold text-purple-400">
              {currencyFormatter.format(salesByProduct["จักรยานไฟฟ้า"] || 0)}
            </p>
            <p className="text-xs text-muted mt-1">
              {totalSales > 0 ? ((salesByProduct["จักรยานไฟฟ้า"] || 0) / totalSales * 100).toFixed(1) : 0}% ของยอดขาย
            </p>
          </div>
          <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-4">
            <p className="text-sm text-muted mb-1">สามล้อไฟฟ้า</p>
            <p className="text-2xl font-bold text-green-400">
              {currencyFormatter.format(salesByProduct["สามล้อไฟฟ้า"] || 0)}
            </p>
            <p className="text-xs text-muted mt-1">
              {totalSales > 0 ? ((salesByProduct["สามล้อไฟฟ้า"] || 0) / totalSales * 100).toFixed(1) : 0}% ของยอดขาย
            </p>
          </div>
          <div className="bg-cyan-500/10 border border-cyan-500/30 rounded-xl p-4">
            <p className="text-sm text-muted mb-1">อะไหล่ EV</p>
            <p className="text-2xl font-bold text-cyan-400">
              {currencyFormatter.format(salesByProduct["อะไหล่ EV"] || 0)}
            </p>
            <p className="text-xs text-muted mt-1">
              {totalSales > 0 ? ((salesByProduct["อะไหล่ EV"] || 0) / totalSales * 100).toFixed(1) : 0}% ของยอดขาย
            </p>
          </div>
        </div>
      </motion.div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              >
                ...
                <option value="ผ่อน 0% 12 เดือน">ผ่อน 0% 12 เดือน</option>
